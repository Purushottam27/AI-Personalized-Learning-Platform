...
Represents the time of day during which the learner generally prefers to study.
...
Represents how long a learner generally prefers an individual learning session to be.
...
Represents the days on which the learner generally prefers or expects to study.
...
Notification timing should eventually be based on the learner's study preferences and actual learning behavior rather than sending generic notifications.
...
# 6. Instructor Verification & Trusted Instructor Creation**
...
For the current MVP, public registration may explicitly select `LEARNER` or `INSTRUCTOR`.

The system must always reject or prevent public creation of an `ADMIN` account. Admin accounts are provisioned separately.

## Future Enhancement**

A later version may introduce a trusted mechanism for instructor account creation or verification.

Possible future mechanisms include:

- administrator approval,

- invitation-based instructor registration,
...
This must be decided before implementing the future instructor-verification system.

---


## Current MVP Foundations Referenced by This Register

The following are already approved/implemented MVP foundations and should not be treated as deferred features:

- RefreshSession persistence
- Account suspension/deactivation session revocation
- Current-user status checks on protected requests
- Account reactivation
- Learning-progress preservation during deactivation
- Public LEARNER/INSTRUCTOR role selection
- Separate AuthIdentity and temporary OAuth setup-session architecture

This document records future extensions around these foundations; it does not reopen their finalized MVP decisions.
...
## RefreshSession Architecture Reference**
...
These fields are part of the finalized MVP authentication foundation.
...
reactivation flow after receiving `ACCOUNT\_DEACTIVATED`.
...
The current MVP personalization approach should avoid overwhelming learners with a large list of recommendations.
...
- invitation-based instructor creation,

- institutional instructor verification,
...
# 12A. Deferred Schema Discipline

Deferred fields must not be added to the MVP schema merely to make future features easier.
...
should remain absent from the active MVP learner profile until study scheduling is formally activated.

Likewise, future authentication capabilities should not introduce unused fields or endpoints into the MVP solely as placeholders.

When a deferred feature is activated, its data model should be designed against the then-current architecture rather than assuming today's design will remain unchanged.
...
\| Trusted instructor creation | Deferred |

\| Instructor verification | Deferred |
