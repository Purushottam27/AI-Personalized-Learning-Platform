# AI Based Personalized Learning Platform — Learner Learning Model

## 1. Purpose**

The Learner Learning Model defines how the platform represents,
...
The Learner is the learner using the platform.

Learner information has two conceptual categories.
...
The learner's account state affects access to learning without destroying
...
Do NOT imply that the system generates the learner's entire future
...
As the learner learns, the profile can contain:
...
## 4A. Finalized Learner Profile Structure

The learner-specific profile is separate from `User`.
...
Account identity information such as name, email, avatar, role, and account
status remains in `User`.

The MVP does not add `preferredTime`, `preferredSessionDuration`, or
`studyDays`; those belong to future study-scheduling functionality.

---

## 4B. Instructor Profile Boundary

Instructor-specific professional information is represented separately in
`InstructorProfile`:
...
This document focuses on the learner model, but instructor-owned course
content and professional profile information must not be conflated with
the learner's learning profile.

---

## 5. Course**

A Course is a structured learning program created by a Instructor.
...
A Diagnostic Assessment determines whether a learner has sufficient

prerequisite knowledge.

It is course-specific and instructor-configured.
...
If the learner already completed the prerequisite course, targeted
...
Enrollment represents the relationship between a Learner and a Course.
...
Topic-level performance is important because a learner may have
...
Instructor-provided resources may include:
...
Its purpose is to help the learner understand and strengthen a concept.
...
Assessment measures learner understanding.
...
Initial instructor question creation methods:
...
The learner should not automatically be required to repeat the entire
...
The recommendation engine combines deterministic personalization rules
and AI reasoning. Deterministic constraints remain authoritative.
...
learner's learning state.

## 31A. Intervention Selection

Personalization should select interventions using the approved hierarchy:
...
The engine should not force a learner through a fixed sequence such as
course → topic → lesson → resource → practice → assessment when a more
specific intervention is appropriate.

For example, a weakness isolated to Lesson 7 should prefer a targeted
Lesson 7 intervention rather than unnecessarily recommending the entire
course.
...
A learner studies Normalization.
...
## 34A. Shared Learning Taxonomy

Learner interests and instructor expertise use the same high-level learning
domains in the MVP:
...
This shared taxonomy can later connect learner interests, instructor
expertise, course domains, and recommendation/discovery logic.
...
9\.  Instructor content remains important.
...
- AI-generated assessments without instructor approval
